...
public class LoginPage extends Composite {
	...
	public LoginPage() {
		...
		// Versuch die Klasse LoginFotos als Konstruktor aufzurufen, da sich die
		// Login-kästchen und Bilder in DecoratorPanel überlappen
		...
		// Dem DecoratorPanel die Flextable hinzufügen und den DecoratorPanel
		// wiederrum dem HorizontalPanel hinzufügen
	...
	private class PowerButtonHandler implements ClickHandler {
		...
		public void onClick(ClickEvent event) {
			...
			if (eingabeBenutzerName.getText().length() == 0
					&& eingabeBenutzerPasswort.getText().length() == 00) {
			...
			if (eingabeBenutzerName.getText().equals(new String("Feridun"))) {
				...
				// Versuch den Konstruktor von HauptLayout hier aufzurufen, so
				// dass beim Klicken auf anmelden der Raumplaner geöffnet wird

				hl1 = new HauptLayout();
				RootPanel.get().add(hl1);
				loginPanel2.setVisible(true);

				// Funktioniert gerade noch auf der LoginPage. Dieser Bug wird
				// demnächst behoben --> ERLEDIGT
